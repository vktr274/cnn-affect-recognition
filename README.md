...
In conclusion, the dataset would need to contain more images with more variations of the same expression or, in addition to that, it would need to be preprocessed with different techniques, e.g. background removal, watermark removal, etc. Another option might be that the images are already well preprocessed and the labels are inconsistent. In that case, the labels would need to be improved. After a glance at the improved labels, we didn't find any obvious mistakes, although there seem to be some images that could be interpreted as different expressions which is a complex and difficult problem to model with a neural network well. We note that we didn't investigate the labels thoroughly considering the size of the dataset.
