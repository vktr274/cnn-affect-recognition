...
We also used a similar learning rate schedule as in the paper. Our scheduler was set to monitor validation loss and reduce the learning rate by a factor of 0.1 when the validation loss did not improve for 5 epochs. We also used early stopping with a patience of 10 epochs and best weights restoration. The loss function we used throughout all experiments was sparse categorical crossentropy.
...
Based on the last training the previously observed issues are likely to be caused by not enough data. To create more variations of the data we can use a `Sequential` model with preprocessing layers to create more image variations on the fly. We defined the following Sequential model:
...
To achieve reasonable results, the dataset would need to contain more images with more variations of the same expression or, in addition to that, it would need to be preprocessed with different techniques, e.g. background removal, watermark removal, etc. Another option might be that the images are already well preprocessed and the labels are inconsistent. In that case, the labels would need to be improved.

After a glance at the improved labels, we didn't find any obvious mistakes, although there seem to be some images that could be interpreted as different expressions which is a complex and difficult problem to model with a neural network well. We note that we didn't investigate the labels thoroughly considering the size of the dataset. Improving the dataset by changing labels or removing unclear images would require a lot of manual work and would be very time consuming.

In conclusion, classifying facial expressions, especially if their interpretation is unclear, is a very difficult task and the dataset we used is not good enough to train a model that can recognize face expressions well.
